    ...
    m_drivetrain.setDefaultCommand(
      ...
      m_drivetrain.applyRequest(() -> 
        drive.withVelocityX(-m_driverController.getLeftY() * DriveConstants.MaxSpeed) // Drive forward with negative Y (forward)
          ...
          .withRotationalRate(m_drivetrain.getAdjustedRotation(-MathUtil.applyDeadband(m_driverController.getRightX(),0.1)) * DriveConstants.MaxAngularRate) // Drive counterclockwise with negative X (left)
      )
    );
